    ...
    <div className="glass-card overflow-hidden">
      ...
      <div className="border-b border-gray-700/40 px-5 py-5 text-center">
        <div className="mx-auto mb-3 flex h-16 w-16 items-center justify-center rounded-full bg-gradient-to-br from-synapse-neon-purple to-indigo-600 shadow-glow-purple">
          <span className="text-2xl">{pct >= 0.8 ? '🌟' : pct >= 0.5 ? '👍' : '📚'}</span>
        </div>
      ...
      <div className="divide-y divide-gray-700/30 px-5">
        {questions.map((q, i) => {
          ...
          const isCorrect =
            q.is_correct ?? userAns.toLowerCase().trim() === correct.toLowerCase().trim();
          ...
          return (
            <div key={q.id} className="py-4">
              <div className="mb-2 flex items-start gap-2">
                {isCorrect ? (
                  <svg
                    className="mt-0.5 h-4 w-4 shrink-0 text-synapse-neon-green"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    strokeWidth={2.5}
                  ...
                  <svg
                    className="mt-0.5 h-4 w-4 shrink-0 text-synapse-neon-red"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    strokeWidth={2.5}
                ...
                <div className="min-w-0 flex-1">
                        ...
                        return (
                          <div key={oi} className={`flex items-center gap-2 text-xs ${optStyle}`}>
                            ...
                            {isOptCorrect && isOptSelected && (
                              <span className="text-synapse-neon-green">✓</span>
                            )}
                            {isOptCorrect && !isOptSelected && (
                              <span className="rounded border border-synapse-neon-green/20 bg-synapse-neon-green/10 px-1 text-[9px] text-synapse-neon-green">
                                correct
                              </span>
                            )}
                          </div>
                  ...
                  {!isMcq && (
                    <div className="mt-2 space-y-1">
                      <p
                        className={`text-xs ${isCorrect ? 'text-synapse-neon-green' : 'text-synapse-neon-red'}`}
                      >
                        Your answer: {userAns || '(none)'}
                      </p>
                      {!isCorrect && (
                        <p className="text-xs text-synapse-neon-green">Correct: {correct}</p>
                      )}
                    </div>
                  )}
                </div>
