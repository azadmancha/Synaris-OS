    ...
    <header className="flex items-center justify-between border-b border-gray-200 bg-white/80 px-4 py-3 backdrop-blur-sm dark:border-gray-700 dark:bg-[#0F1117]/80">
      ...
      <div className="flex items-center gap-2">
        ...
        {user && (
          <a
            href="/settings"
            className="flex items-center gap-2 rounded-lg px-2 py-1 text-xs text-gray-400 transition-colors hover:bg-gray-50 hover:text-gray-600 dark:hover:bg-[#1C1E2B] dark:hover:text-gray-300"
          >
            {user.avatarUrl ? (
              <img
                src={user.avatarUrl}
                alt=""
                className="h-6 w-6 rounded-full ring-2 ring-gray-200 dark:ring-gray-700"
              />
            ) : (
