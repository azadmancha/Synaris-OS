...
class TestHealthPing:
    ...
    def test_ping_has_no_db_dependency(self, client: TestClient):
...
class TestHealthCheck:
    ...
    def test_health_database_check_exists(self, client: TestClient):
        """Health should include a database connectivity check."""
        response = client.get("/v1/health")
        data = response.json()
        assert "database" in data.get("checks", {})

    def test_health_database_is_healthy(self, client: TestClient):
        """Database should report healthy with in-memory SQLite."""
        response = client.get("/v1/health")
        data = response.json()
        assert data["checks"]["database"] == "healthy"

    def test_health_ai_providers_check_exists(self, client: TestClient):
        """Health should include AI provider check."""
        response = client.get("/v1/health")
        data = response.json()
        assert "ai_providers" in data.get("checks", {})

    def test_health_ai_providers_has_available_list(self, client: TestClient):
    ...
    def test_health_api_keys_check_exists(self, client: TestClient):
        """Health should include API key configuration check."""
        response = client.get("/v1/health")
        data = response.json()
        assert "api_keys" in data.get("checks", {})

    def test_health_api_keys_has_gemini_groq_openrouter(self, client: TestClient):
    ...
    def test_health_status_degraded_without_ai(self, client: TestClient):
